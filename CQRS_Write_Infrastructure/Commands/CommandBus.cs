...
namespace CQRS_Write_Infrastructure.Commands
{
    public class CommandBus : ICommandBus
    {
        ...
        {
            var commandHandlerMethod = commandHandler.GetType().GetMethods().Where(m => m.GetParameters().Any(p => p.ParameterType.GetInterfaces().Contains(typeof(ICommand))));
            foreach (var method in commandHandlerMethod)
            {
                ParameterInfo commandParameterInfo = method.GetParameters().Where(p => p.ParameterType.GetInterfaces().Contains(typeof(ICommand))).FirstOrDefault();
        ...
        {
            var commandHandlerMethod = eventHandler.GetType().GetMethods().Where(m => m.GetParameters().Any(p => p.ParameterType.GetInterfaces().Contains(typeof(IEvent))));
            foreach (var method in commandHandlerMethod)
            {
                ParameterInfo commandParameterInfo = method.GetParameters().Where(p => p.ParameterType.GetInterfaces().Contains(typeof(IEvent))).FirstOrDefault();
                if (commandParameterInfo == null) continue;
